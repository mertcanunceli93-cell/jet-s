...
export default function Addresses() {
  ...
  const handleAddAddress = async () => {
    const title = prompt("Adres başlığı (örn. Ev, İş):");
    if (!title) return;
    const fullAddress = prompt("Açık adres:");
  ...
  const getIcon = (title: string) => {
    ...
    if (t.includes('iş') || t.includes('ofis')) return Briefcase;
    return Star;
  };
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-fade-up">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button 
          ...
          className="px-8 py-4 bg-gradient-to-r from-brand-orange to-[#ff9d42] text-white rounded-2xl font-black hover:shadow-[0_0_30px_rgba(255,122,0,0.4)] transition-all flex items-center gap-2 hover:scale-[1.02] active:scale-95 group"
        >
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
        ...
        <button 
          onClick={handleAddAddress}
          className="glass-card border-dashed p-8 flex flex-col items-center justify-center text-gray-500 hover:text-brand-orange hover:bg-white/[0.04] hover:border-brand-orange/30 transition-all group min-h-[300px]"
        >
