...
import {
  Package, Home, Clock, Settings,
  TrendingUp, Users, ChevronLeft, LogOut, Zap,
  Truck, BarChart3, Bell, ScrollText
} from 'lucide-react';
...
export default function Sidebar({ mobileOpen, onClose }: { mobileOpen?: boolean; onClose?: () => void }) {
  ...
  const getNavItems = () => {
    return [
      { name: 'Sistem Özeti',  href: '/dashboard',              icon: Home       },
      { name: 'Tüm Siparişler',href: '/dashboard/all-orders',   icon: Package    },
      { name: 'Kuryeler',      href: '/dashboard/couriers',     icon: Truck      },
      { name: 'Analitik',      href: '/dashboard/analytics',    icon: BarChart3  },
      { name: 'Pricing Engine',href: '/dashboard/pricing',      icon: TrendingUp },
      { name: 'Kullanıcılar',  href: '/dashboard/users',        icon: Users      },
      { name: 'Bildirimler',   href: '/dashboard/notifications',icon: Bell       },
      { name: 'Aktiviteler',   href: '/dashboard/history',      icon: Clock      },
      { name: 'Sistem Günlüğü',href: '/dashboard/audit-log',    icon: ScrollText },
